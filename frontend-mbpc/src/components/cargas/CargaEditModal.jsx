  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.put(
        ...
        {
          barcazaId: Number(data.barcazaId),
          tipo: data.tipo,
          ...
          <div className="space-y-5 px-6 py-5">
            ...
            <div>
              ...
              <input
                ...
                type="number"
                step="1"
                className={`w-full rounded-md border px-3 py-2 text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500 transition ${
                  errors.barcazaId
                ...
                placeholder="Ej: 1042"
                {...register("barcazaId", {
                  required: "El ID de barcaza es requerido.",
                  min: { value: 1, message: "Debe ser un entero positivo." },
                  validate: (v) =>
                    Number.isInteger(Number(v)) || "Debe ser un número entero.",
                })}
              />
              {errors.barcazaId && (
                <p className="mt-1 text-xs text-red-500">
                  {errors.barcazaId.message}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <select
                id="tipo"
                className={`w-full rounded-md border px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 transition bg-white ${
                  errors.tipo
                    ? "border-red-400 focus:ring-red-400"
                    : "border-slate-300"
                }`}
